...
from sapien_env.rl_env.pen_insertion_contact_env import PenInsertionContactRLEnv
...
class SingleArmPolicy:
    ...
    def generate_trajectory(self, env , ee_link_pose, mode='straight'):
        ...
        #     pen_position = pen_pose.p+np.array([0.,0.,0.105])
        ...
        # else:
        #     pen_position = pen_pose.p+np.array([0.,0,0.105])
        pen_position = pen_pose.p + np.array([0., 0., env.object_half_length * env.object_scale * 2 + env.cylinder_to_ground - 0.04])
        ...
        # insersion quat is ee_link_pose quat rotated by 90 degrees around x axis
        # insertion_pose_quat = transforms3d.quaternions.qmult(ee_link_pose.q, transforms3d.quaternions.axangle2quat([0, 1, 0], np.pi / 2))
        # insertion_pose_quat = transforms3d.quaternions.qmult(insertion_pose_quat, transforms3d.quaternions.axangle2quat([1, 0, 0], np.pi / 2))

        pencil_holder_2_pose = env.pencil_holder_2.get_pose()
        pencil_holder_2_pose = pencil_holder_2_pose
        print(pencil_holder_2_pose.p)
        # insertion_position = pencil_holder_2_pose.p + np.array([0., 0., env.object_half_length * env.object_scale * 2 + env.cylinder_to_ground - 0.03])
        # pre_insertion_position = pencil_holder_2_pose.p + np.array([0., 0., env.object_half_length * env.object_scale * 2 + env.cylinder_to_ground +0.02])
        insertion_position = np.array([pencil_holder_2_pose.p[0], pencil_holder_2_pose.p[1], pen_position[2] - 0.01])
        pre_insertion_position = np.array([pencil_holder_2_pose.p[0], pencil_holder_2_pose.p[1], pen_position[2] + 0.05])
        print('actual pencil positon', env.manipulated_object.get_pose().p)
        print('pen_position', pen_position)
        print('insertion position', insertion_position)
        print('pre insertion position', pre_insertion_position)
        if mode == 'straight':
            self.trajectory = [
                {"t": 0, "xyz": ee_link_pose.p, "quat": ee_link_pose.q, "gripper": 0.09},
                {"t": 30, "xyz": pen_position + np.array([0, 0, 0.05]), "quat": ee_link_pose.q, "gripper": 0.09},
                {"t": 40, "xyz": pen_position, "quat": ee_link_pose.q, "gripper": 0.09},
                {"t": 60, "xyz": pen_position, "quat": ee_link_pose.q, "gripper": 0.0},
                {"t": 70, "xyz": pen_position, "quat": ee_link_pose.q, "gripper": 0.0},
                {"t": 80, "xyz": pen_position + np.array([0, 0, 0.05]), "quat": ee_link_pose.q, "gripper": 0.0},
                {"t": 140, "xyz": pre_insertion_position, "quat": ee_link_pose.q, "gripper": 0.0},
                {"t": 180, "xyz": insertion_position, "quat": ee_link_pose.q, "gripper": 0.0},
                {"t": 200, "xyz": insertion_position, "quat": ee_link_pose.q, "gripper": 0.0},
            ]
        else:
...
def main_env():
    ...
    success = 0
    success_rate =0
    env = PenInsertionContactRLEnv(manip_obj='pencil',use_gui=True, robot_name="panda", frame_skip=10, use_visual_obs=False)
    env.seed(0)
    env.reset()
    arm_dof = env.arm_dof
