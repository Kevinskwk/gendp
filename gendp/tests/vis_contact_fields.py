...
from gendp.gendp.common.data_utils import load_dict_from_hdf5, d3fields_proc, get_contact_field
...
# Contact visualization parameters
contact_sphere_radius = 0.005  # Size of spheres representing contact points
force_scale = 0.05  # Scaling factor for force vector visualization
min_force_magnitude = 0.01  # Minimum force magnitude to visualize
...
for i in tqdm(epi_range):
    ...
    if vis_action:
        ...
        action_colors = action_cm(np.linspace(0, 1, init_cart.shape[0], endpoint=True))[:, :3]
    ...
    for t in tqdm(range(T*2//3, T)):
        ...
        depths = np.stack([data_dict['observations']['images'][f'{cam_key}_depth'][t:t+1] for cam_key in cam_keys], axis=1) / 1000.
        ...
        pcd_world = pcd.copy()  # Save a copy in world frame for contact mapping
        pcd = np.linalg.inv(robot_base_in_world) @ np.concatenate([pcd, np.ones((pcd.shape[0], 1))], axis=-1).T
        pcd = pcd.T[:, :3]

        if vis_contact:
            # visualize contact field
            # Get contact field for current timestep
            contact_feats = get_contact_field(pcd_world, 
                                            contact_points=data_dict['observations']['contact_points'][t], 
                                            smoothing_radius=0.02,
                                            force_scaling=1.0, 
                                            min_force_magnitude=0.001,
                                            )
            contact_feats_cmap = colormaps.get_cmap('viridis')
            pcd_colors = contact_feats_cmap(contact_feats[:, 0]*255)[:, :3]
        else:
        ...
        if vis_action:
            ...
            ee_target_pose = data_dict['observations']['ee_pos'][t_start:t_end]
