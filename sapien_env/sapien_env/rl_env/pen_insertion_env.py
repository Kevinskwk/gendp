...
from sapien_env.sim_env.pen_insertion_env import PenInsertionEnv
...
class PenInsertionRLEnv(PenInsertionEnv, BaseRLEnv):
    ...
    def get_reward(self, action):
        ...
        reward = (obj_pose.p[2] > h_thresh_lower) and obj_rot[1,2] > 0.8
        print(obj_rot)
        print(obj_pose.p)
        return float(reward)
...
def main_env():
    env = PenInsertionRLEnv(use_gui=True, robot_name="panda", frame_skip=10, use_visual_obs=False)
    base_env = env
