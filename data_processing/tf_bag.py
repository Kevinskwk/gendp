...
def suppress_tf_warnings():
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message=".*TF_REPEATED_DATA.*")
        yield
    ...
    def lookupTransform(self, orig_frame, dest_frame, time):
        ...
        with suppress_tf_warnings():
            try:
                ...
                return self.transformer.lookupTransform(orig_frame, dest_frame, common_time - rospy.Duration(0.07))

    def lookupTwist(self, orig_frame, dest_frame, time):
        """
        Returns the twist between the two provided frames at the given time
        ...
        try:
            return self.transformer.lookupTwist(orig_frame, dest_frame, common_time, rospy.Duration(0.1))
        except tf.ExtrapolationException:
            return self.transformer.lookupTwist(orig_frame, dest_frame, common_time - rospy.Duration(0.12), rospy.Duration(0.1))
