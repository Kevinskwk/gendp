...
from sapien_env.sim_env.pen_insertion_contact_env import PenInsertionContactEnv
...
class PenInsertionContactRLEnv(PenInsertionContactEnv, BaseRLEnv):
    ...
    def get_reward(self, action):
        ...
        obj_rot = obj_pose.to_transformation_matrix()[:3, :3]
        
        tip_to_ground = 0.03

        xy_thresh = 0.05
        
        # Compute xy distance between object and pencil_holder
        pencil_holder_xy = self.pencil_holder_2.get_pose().p[:2]
        obj_xy = obj_pose.p[:2]
        xy_dist = np.linalg.norm(obj_xy - pencil_holder_xy)

        reward = (obj_pose.p[2] < self.object_half_length * self.object_scale + self.cylinder_to_ground + 0.02) and \
                (obj_rot[2, 2] < -0.9) and \
                (xy_dist < xy_thresh)

        # print('rot', obj_rot)
        # print('pos',obj_pose.p)
        # print('height_thresh', self.object_half_length * self.object_scale + self.cylinder_to_ground + 0.02)
        # print('reward', reward)
        return float(reward)
    ...
    def set_init(self, init_states):
        ...
        self.pencil_holder_2.set_pose(init_box_pose)
...
def main_env():
    env = PenInsertionContactRLEnv(use_gui=True, robot_name="panda", frame_skip=10, use_visual_obs=False)
    base_env = env
